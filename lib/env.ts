...
const RawEnvSchema = z.object({
    ...
    if (!data.SUPABASE_URL && !data.NUXT_PUBLIC_SUPABASE_URL) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Missing SUPABASE_URL or NUXT_PUBLIC_SUPABASE_URL",
            path: ["SUPABASE_URL"],
        });
    }
    if (!data.SUPABASE_KEY && !data.NUXT_PUBLIC_SUPABASE_ANON_KEY) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Missing SUPABASE_KEY or NUXT_PUBLIC_SUPABASE_ANON_KEY",
            path: ["SUPABASE_KEY"],
        });
    }
});
...
export const env = {
    supabaseUrl: parsed.data.SUPABASE_URL ?? parsed.data.NUXT_PUBLIC_SUPABASE_URL!,
    supabaseKey: parsed.data.SUPABASE_KEY ?? parsed.data.NUXT_PUBLIC_SUPABASE_ANON_KEY!,
